...
//    private final String groupId = "SENTINEL_GROUP";
    ...
    private final String apolloNamespace = "sentinel";
    ...
    public void init() throws Exception {
        ...
        // Token server global flow config extracted from assign map
        initGlobalServerFlowProperty();
    ...
    private void initDynamicRuleProperty() {
        ReadableDataSource<String, List<FlowRule>> ruleSource = new ApolloDataSource<>(apolloNamespace,
                flowDataId,"[]",source -> JSON.parseObject(source, new TypeReference<List<FlowRule>>() {})
                );

        FlowRuleManager.register2Property(ruleSource.getProperty());

        ReadableDataSource<String, List<ParamFlowRule>> paramRuleSource = new ApolloDataSource<>(apolloNamespace,
                paramDataId,"[]",source -> JSON.parseObject(source, new TypeReference<List<ParamFlowRule>>() {}));
        ParamFlowRuleManager.register2Property(paramRuleSource.getProperty());
    }

    private void initClientConfigProperty() {
        ReadableDataSource<String, ClusterClientConfig> clientConfigDs = new ApolloDataSource<>(apolloNamespace,
            configDataId,"{}", source -> JSON.parseObject(source, new TypeReference<ClusterClientConfig>() {}));
        ClusterClientConfigManager.registerClientConfigProperty(clientConfigDs.getProperty());
    }

    private void initServerTransportConfigProperty() {
        ReadableDataSource<String, ServerTransportConfig> serverTransportDs = new ApolloDataSource<>(apolloNamespace,
    ...
    private void initGlobalServerFlowProperty() {
        ReadableDataSource<String, ServerFlowConfig> serverFlowDs = new ApolloDataSource<>(apolloNamespace,
                clusterMapDataId, "{}", source -> {
            List<ClusterGroupEntity> groupList = JSON.parseObject(source, new TypeReference<List<ClusterGroupEntity>>() {});
            return Optional.ofNullable(groupList)
    ...
    private void registerClusterRuleSupplier() {
        ...
        ClusterFlowRuleManager.setPropertySupplier(namespace -> {
            ReadableDataSource<String, List<FlowRule>> ds = new ApolloDataSource<>(apolloNamespace,
        ...
        ClusterParamFlowRuleManager.setPropertySupplier(namespace -> {
            ReadableDataSource<String, List<ParamFlowRule>> ds = new ApolloDataSource<>(apolloNamespace,
                    namespace + DemoConstants.FLOW_POSTFIX, "[]",
    ...
    private Optional<ServerFlowConfig> extractServerFlowConfig(List<ClusterGroupEntity> groupList) {
        return groupList.stream()
                .filter(this::machineEqual)
                .findAny()
                .map(e -> new ServerFlowConfig(APP_NAME).setMaxAllowedQps(e.getGlobalFlow()));
    }
